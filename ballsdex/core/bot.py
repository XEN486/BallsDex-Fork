...
PACKAGES = [
    f.name for f in (Path(__file__).resolve().parent.parent / "packages").iterdir()
]
...
class BallsDexBot(commands.AutoShardedBot):
    ...
    def __init__(
        self, command_prefix: PrefixType[BallsDexBot], dev: bool = False, **options
    ):
    ...
    async def on_command_error(
        ...
        if isinstance(exception, commands.CommandInvokeError):
            ...
            log.error(
                f"Unknown error in text command {context.command.name}",
                exc_info=exception,
            )
